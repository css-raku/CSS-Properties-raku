...
unit class CSS::Properties::Calculator;
...
method reference-width is rw {
    sub FETCH($) { $!reference-width // 0 }
    sub STORE($, Numeric:D() $v) {
...
my constant OpenParen  = %( :op<(> );
my constant CloseParen = %( :op<)> );
multi sub calc(OpenParen, %expr, CloseParen ) {
    calc %expr;
}
